...
export async function fetchAppRemoteConfiguration(
  ...
  const appModuleVersionsConfig =
    activeAppVersion?.appModuleVersions.filter((module) => module.specification?.experience === 'configuration' || specifications.find((spec) => spec.identifier === module.specification?.identifier && spec.extensionManagedInToml)) || []
...
export function remoteAppConfigurationExtensionContent(
  ...
  const configSpecifications = specifications.filter((spec) => spec.experience === 'configuration' || spec.extensionManagedInToml)
